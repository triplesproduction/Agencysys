...
export default function EmployeesPage() {
    ...
    // Single clean fetch effect — fires whenever ANY query param changes
    useEffect(() => {
        let cancelled = false;
        const doFetch = async () => {
            setLoading(true);
            try {
                ...
                if (!user || (!user.role?.includes('ADMIN') && !user.roleId?.includes('ADMIN'))) {
    ...
    }, [page, debouncedSearch, roleFilter, deptFilter, statusFilter, refreshKey]);
    ...
    const handleDeleteEmployee = async (emp: EmployeeDTO, e: React.MouseEvent) => {
        ...
        try {
            ...
            addNotification({ title: 'Employee Removed', message: `Personnel record for ${emp.firstName} ${emp.lastName} has been deleted.`, type: 'SUCCESS', metadata: null });
        } catch (err: any) {
            ...
            <div className="emp-page-header-alt">
                <div className="emp-count-block">
                    ...
                    <div className="emp-count-stats">
                        <span><div className="dot active"></div> Active {employees.filter(e => e.status === 'ACTIVE').length}</span>
                        <span><div className="dot inactive"></div> Inactive {employees.filter(e => e.status !== 'ACTIVE').length}</span>
                    </div>
                </div>
